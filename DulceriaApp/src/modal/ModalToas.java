...
public class ModalToas extends Modal implements ModalBorderAction {
    ...
    private void initLogo(Type type) {
        switch (type) {
            case SUCCESS -> changeLogo("resources/icon/toast/ic_success.svg", Type.SUCCESS.getValue());
            case WARNING -> changeLogo("resources/icon/toast/ic_warning.svg", Type.WARNING.getValue());
            case ERROR -> changeLogo("resources/icon/toast/ic_error.svg", Type.ERROR.getValue());
            case INFO -> changeLogo("resources/icon/toast/ic_info.svg", Type.INFO.getValue());
        }
    }
    ...
    public void installComponent() {
        ...
        add(message, "grow,w 370!,gapy 10");
        insertsButtons();
        updateUI();
        revalidate();
    }
    ...
    private void insertsButtons() {
        ...
        switch (type) {
            case INFO -> btnOther.setText("Volver");
            case WARNING -> btnOther.setText("Cancelar");
            default -> btnOther.setVisible(false);
        }
    }
    ...
    private ModalController createController() {
        return new ModalController(this) {
            public void close() {
                ModalToas.this.getController().closeModal();
            }
        };
    }
